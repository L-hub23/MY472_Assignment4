...
```{r}
file_checker(db, "Left_Right_Scale")
```
...
constituency_filtered <- constituency_wages %>%
  filter(Constituency %in% mp_leftright_filtered$Constituency)

constituency_filtered

dbWriteTable(db, "Constituency_Med_Wages", constituency_filtered)
```

```{r, eval=TRUE}
file_checker(db, "Constituency_Med_Wages")
```
